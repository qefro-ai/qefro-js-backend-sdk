...
node server.js >/tmp/qefro-smoke-flows.log 2>&1 &
SERVER_PID=$!
...
echo "${CAPABILITIES_RESPONSE}" | grep -q '"tools":'
echo "${CAPABILITIES_RESPONSE}" | grep -q "\"id\":\"${FLOW_ID}\""
echo "${CAPABILITIES_RESPONSE}" | grep -q '"tool_ref":"lookup_customer"'

echo "Smoke test passed for ${FLOW_ID}"
